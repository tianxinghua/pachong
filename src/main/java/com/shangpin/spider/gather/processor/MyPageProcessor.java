						...
						if(spiderRuleInfo.getXdetailUrlStrategy().equals(StrategyConstants.C)) {
							...
							for (String detailRule : gg) {
								if(detailRule.contains(SymbolConstants.ATTR_FLAG)){
									...
									Elements elements = page.getHtml().getDocument().select(spiderRuleInfo.getXdetailUrlRules());
									...
									List<String> list = page.getHtml().css(spiderRuleInfo.getXdetailUrlRules()).links().all();
