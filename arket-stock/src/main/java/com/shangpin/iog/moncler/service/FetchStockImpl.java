...
public class FetchStockImpl
{
    ...
    public void fetchItlyProductStock()
    {
        ...
        System.out.println("============拉取arket库存信息开始 " + startDateTime + "=========================");
    ...
    private static boolean solveProductQty(ProductDTO productDTO)
    {
        ...
        try
        {
            ...
            if (response.getStatus() == 200)
            {
                ...
                Elements temSizeElements = doc.select("div.product-size-wrapper").select("ul#size").select("li");
                for (int i=0;i<zhiCaiSkuResultListSize;i++){
                    SkuDTO skuDTO = zhiCaiSkuResultList.get(i);
                    String spSizeName=skuDTO.getSize();
                    for (int j=0;j<temSizeElements.size();j++){
                        String temQty=NO_STOCK;
                        String tempSize=temSizeElements.get(j).text();
                        if (!(spSizeName.equalsIgnoreCase(tempSize))){

                            if (j==temSizeElements.size()-1){
