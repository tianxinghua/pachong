...
public class FetchStockImpl {
    ...
    public void fetchItlyProductStock() {
        ...
        System.out.println("============拉取clarks库存数据开始 " + startDateTime + "=========================");
        logger.info("==============拉取clarks库存数据开始 " + startDateTime + "=========================");
        ...
        String temFilePath = filePath + "clarks-qty-" + todayStr + ".csv";
        String priceFilePath = filePath + "clarks-price-"+todayStr+".csv";
        ...
        //获取第一页商品数据
        ShangPinPageContent gucciPageContent = getShangPinPageContentByParam(supplierId, "clarks", 1, Integer.parseInt(pageSize), channel);
        ...
        for (int i = 2; i <= pageNumber; i++) {
            ShangPinPageContent temgucciPageContent = getShangPinPageContentByParam(supplierId, "clarks", i, Integer.parseInt(pageSize), channel);
            if (temgucciPageContent != null) {
                productDTOAllList.addAll(temgucciPageContent.getZhiCaiResultList());
            } else { //请求失败重新 再次请求
                temgucciPageContent = getShangPinPageContentByParam(supplierId, "clarks", i, Integer.parseInt(pageSize), channel);
                if (temgucciPageContent != null) {
                    productDTOAllList.addAll(temgucciPageContent.getZhiCaiResultList());
                }
            }
    ...
    private static boolean solveProductQty(ProductDTO productDTO) {
        ...
        try {
            ...
            if (response.getStatus() == 200) {
                ...
                zhiCai:
                for (int k = 0; k < zhiCaiSkuResultListSize; k++) {
                    skuDTO = zhiCaiSkuResultList.get(k);
                    String spSkuNo = skuDTO.getSpSkuNo();
                    if (spSkuNo == null || "".equals(spSkuNo)) {
                        continue zhiCai;
                    }
                Elements elements= doc.select("div.box-selectors").select("div");
                if(elements!=null) {
                    String json = "";
                    //System.out.println("尺寸的json串："+elements);

                    Element element = elements.get(1);
                    json = element.attr("data-matrix");
                    ...
                    //System.out.println("尺寸的大小："+jsonObject.getString("FIT_7M"));
                    String fit_7m = jsonObject.getString("FIT_7M");
                    String resultJsonStr = fit_7m.substring(1, fit_7m.length() - 1);
                    //System.out.println("尺寸："+resultJsonStr);
                    String[] split = resultJsonStr.split("},");
                    String lineSize = "";
                    String qty = "";
                    String onLinePrice = "";
                    for (int i = 0; i < split.length; i++) {
                        //System.out.println("单个尺寸："+split[i]);

                        String[] split1 = split[i].split(":");
                        String onLineSize = split1[0].substring(7, 10);
                        //System.out.println("线上尺寸："+onLineSize);
                        int sizeNum = Integer.parseInt(onLineSize) / 10;

                        int pointNum = Integer.parseInt(onLineSize) % 10;

                        if (pointNum != 0) {
                            lineSize = sizeNum + "." + pointNum;
                        } else {
                            lineSize = "" + sizeNum;
                        }
                        //System.out.println("线上尺寸："+lineSize);
                        String[] split2 = split[i].split(",");
                        for (int j = 0; j < split2.length; j++) {

                            if (split2[j].contains("availableStockAmount")) {
                            ...
                            if (split2[j].contains("skuPriceTitle")) {
                                String[] split3 = split2[j].split(":");
                                onLinePrice = split3[1].substring(split3[1].indexOf("P") + 1, split3[1].indexOf("C")).trim();

                            }
                        }
                        System.out.println("尺寸：" + lineSize + ";库存：" + qty + ";价格：" + onLinePrice);

                        String size = skuDTO.getSize();
                        System.out.println("本地尺寸：" + size);
                        if (lineSize.equals(size)) {
                        ...
                        if (onLinePrice != null) {
                            ...
                            System.out.println("本地价格：" + marketP);
                            float onLineP = Float.parseFloat(onLinePrice);
                            if (marketPrice != null) {
                                System.out.println("线上价格：" + onLineP);
                                if (marketP != onLineP) {
                    ...
                    if(k==zhiCaiSkuResultListSize-1){
                        copyFile();
                    }
                }
...
//        }
    ...
    public static void sendMail(){
        ...
        try
        {
           ...
           /* long dayTime = 1000*3600*24l;
            ...
            DataSource dataSource = new FileDataSource(bdl.getString("csvFilePath")+"clarks-price-"+todayStr+".csv");
    ...
    protected void getFileToEmail(){
       /* long dayTime = 1000*3600*24l;
        ...
        String fileName=bdl.getString("csvFilePath")+"clarks-price-"+todayDateStr+".csv";
        File file=new File(bdl.getString("csvFilePath")+"clarks-price-"+todayDateStr+".csv");
        try {
    ...
    public static boolean copyFile(){
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd");
        String todayDateStr = simpleDateFormat.format(new Date());
        String fileName = bdl.getString("csvFilePath") + "clarks-qty-" + todayDateStr + ".csv";
        String fileNameCopy = bdl.getString("csvFilePath") + "clarks-qty-" + todayDateStr +"copy"+ ".csv";
    ...
    public static void main(String[] args) {
        ...
        ProductDTO productDTO = new ProductDTO();
        productDTO.setProductUrl("https://www.clarks.co.uk/c/Raharto-Plain/p/26133177");
        ...
        //fetchStock.getShangPinPageContentByParam("2018082802044","clarks",1,20,"www.clarks.co.uk");
