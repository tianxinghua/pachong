 ...
 */
@Component("tessbitFetchProduct")
public class TessbitFetchProduct {
	@Autowired
...
//	ExecutorService exe=Executors.newFixedThreadPool(500);//相当于跑4遍
	...
	private void fetch(String url,int i) throws Exception{
		...
		Elements categorys = doc.select(".product-box").select(".item").select(".col-sm-6").select(".col-md-3");
		
		int j=0;
		for (Element category : categorys) {
			j++;
			Element content = category.select(".img-container").select("a").get(0);
	...
	public  void getUrlList() throws Exception {
		...
		try {
			...
			for(String url:arrUri){
				...
				Document doc = Jsoup.parse(htmlContent);
				Element pageEle  = doc.select(".number").get(0);
				page = pageEle.text();
	...
	private static Map<String,String> getProductUrl(String url) {
		...
		try {
			String htmlContent = HttpUtil45.get(url,new OutTimeConfig(1000*20,1000*20,1000*20),null);
				...
				try{
					Element spuSpan = doc.select(".sku").select("span").get(1);
					spu = spuSpan.text();	
				}catch(Exception e){
				}
