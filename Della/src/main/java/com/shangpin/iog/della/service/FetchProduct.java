...
//			}
			...
			for(Item item:items){
				if(null==item.getSupplier_item_code()) continue;
...
//				sku.setMarketPrice(null==item.getRetail_price()?item.getRetail_price():item.getRetail_price().replaceAll(",", "."));
//				sku.setSupplierPrice(null==item.getYour_price()?item.getYour_price():item.getYour_price().replaceAll(",", "."));
//				sku.setSalePrice(null==item.getSold_price()?item.getSold_price():item.getSold_price().replaceAll(",", "."));
//				sku.setProductCode(null==item.getSupplier_item_code()?item.getSupplier_item_code():item.getSupplier_item_code().replaceAll("\"", ""));
//				sku.setColor(null==item.getColor_description()?item.getColor_description():item.getColor_description().replaceAll("\"", ""));
//				sku.setProductDescription(null==item.getItem_detailed_info()?item.getItem_detailed_info():item.getItem_detailed_info().replaceAll(",", ".").replaceAll("\"", ""));
////				String size = item.getSize();
////				if(size.contains("1/2")){
////					size = size.replace("1/2", ".5");
////				}
////				size = item.getSize().replaceAll("\"", "").replaceAll(",", ".");
//				sku.setProductSize(null==item.getSize()?item.getSize():item.getSize().replaceAll("\"", "").replaceAll(",", "."));
