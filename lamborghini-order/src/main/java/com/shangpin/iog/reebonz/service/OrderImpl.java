...
public class OrderImpl extends AbsOrderService {
		...
		if ("400".equals(code)) {
			...
			if ("400".equals(result.getReqCode())) {
				...
				if ("StoreCode not valid".equals(result.getResult())) {
					...
					t.start();
				} else if ("Order number just placed"
					...
					try {
						SendMail.sendMessage(
								...
								"推送efashion订单" + orderDTO.getSpOrderId()
										+ "出现错误,已置为不做处理，原因："
										+ orderDTO.getExcDesc(), messageType);
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
	...
	private String getJsonData(OrderDTO orderDTO) {
		...
		try {
			...
			item.setPurchase_price((Double.parseDouble(price)) + "");
			Item[] i = { item };
			obj.setItems(i);
	...
	public static void main(String[] args) {
		...
		String d = "57737d3774f759a70dda7bd5-37:1";
		orderDTO.setDetail(d);
		orderDTO.setSpOrderId("2016021011");
