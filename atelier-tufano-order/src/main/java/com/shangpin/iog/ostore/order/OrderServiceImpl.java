...
//				}
				...
				if(!HttpUtil45.errorResult.equals(stockData)){
					...
					if(stock > 0){
						//下单
						String returnData = "OK";//newOrderMarketPlace(id_order_mrkp,barcode,qty);
...
//					orderDTO.setStatus(OrderStatus.NOHANDLE); 
	...
	private void sendMail(String messageText){
		try {
			...
			SendMail.sendGroupMail(smtpHost, from, fromUserPassword, to, "!!! tufano订单推送异常", messageText , messageType);
