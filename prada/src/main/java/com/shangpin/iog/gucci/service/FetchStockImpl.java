        ...
        try {
            ...
            if (response.getStatus() == 200) {
                ...
                zhiCai:for (int i = 0; i < zhiCaiSkuResultListSize; i++) {
                    ...
                    Elements sizeSelect1 = doc.select("ul.options>li");
                    // System.out.println("尺寸select："+sizeSelect1);
                    if(sizeSelect1!=null&&!"".equals(sizeSelect1)&&sizeSelect1.size()!=0){
                        for (int j = 0; j <sizeSelect1.size() ; j++) {
                            Element element = sizeSelect1.get(j);
                            onLineSize = element.text().replaceAll(",",".");
                            System.out.println("尺寸："+onLineSize);
                            sku = element.attr("data-sku");
                            System.out.println("sku:"+sku);
                            qty = getQty(sku);
                            if(onLineSize.equals(size)){
                                exportSpSkunoAndQty(spSkuNo,qty);
                            }
                            System.out.println("有尺码的库存："+qty);

                        }
                        System.out.println("尺寸集合："+sizeMap.toString());
                    }else {
                        Element select3 = doc.select("div.component-productDetails").first();
                        if (select3 != null && !"".equals(select3)) {
                            sku = select3.attr("data-sku");
                            qty = getQty(sku);
                            if ("U".equals(size)) {
                                exportSpSkunoAndQty(spSkuNo, qty);
                            }
                            System.out.println("没有尺码的库存：" + qty);
                        }
                        ;
                    }
                    String onLinePrice="";
                    Element priceN = doc.select("div.price-row.text-center.text-md-right.col-12.col-md-5 > div ").first();
                    System.out.println("价格select："+priceN);
                    if(priceN != null && !"".equals(priceN)){
                        String priceArr = priceN.text();
                        onLinePrice = priceArr.replace("€","").trim().replaceAll(",","").trim();
...
//        }
    ...
    public static void main(String[] args) {
       ...
       // fetchStock.getShangPinPageContentByParam("2018082702041","moncler ",1,20,"www.net-a-porter.com");
