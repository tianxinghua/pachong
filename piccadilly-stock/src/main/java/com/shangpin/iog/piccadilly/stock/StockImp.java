package com.shangpin.iog.piccadilly.stock;
...
public class StockImp  extends AbsUpdateProductStock {
    ...
    public Map<String,String> grabStock(Collection<String> skuNo) throws ServiceException, Exception {
    	...
    	int num = 0;
    	String data = "";
    	String skuData = HttpUtil45.postAuth(url+"GetAllAvailabilityMarketplace",null,new OutTimeConfig(1000*60*120,1000*60*120,1000*60*120),"shangpin", "piccadilly");
    ...
    public static void main(String[] args) throws Exception {
    	//加载spring
        loadSpringContext();
        StockImp stockImp =(StockImp)factory.getBean("piccadillystock");
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm");
        logger.info("piccadillystock更新数据库开始");
        try {
			stockImp.updateProductStock(supplierId, "2015-01-01 00:00", format.format(new Date()));
//        	stockImp.updateProductStock(host, app_key, app_secret, "2015-01-01 00:00", format.format(new Date()));
        } catch (Exception e) {
			logger.info("piccadillystock更新库存数据库出错"+e.toString());
		}
        logger.info("piccadillystock更新数据库结束");
        System.exit(0);
    }
}
