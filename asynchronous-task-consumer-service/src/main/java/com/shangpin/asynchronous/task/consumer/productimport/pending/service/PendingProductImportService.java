...
public class PendingProductImportService {
	...
	public static void main(String[] args) {
		ProductImportTask t = new ProductImportTask();
		t.setTaskNo("20161220");
		try {
			new PendingProductImportService().handMessage(t);
		} catch (Exception e) {
	...
	public void handMessage(ProductImportTask task) throws Exception{
		...
		// 2、从ftp下载文件并解析成对象
		List<HubPendingProductImportDTO> listHubProduct = handleHubExcel(task.getTaskFtpFilePath());
		...
		FTPClientUtil.uploadFile(new File(resultFileName+".xlsx"), resultFileName+".xlsx");
	...
	//解析excel转换为对象
	private List<HubPendingProductImportDTO> handleHubExcel(String taskFtpFilePath)  throws Exception {
