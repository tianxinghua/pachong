...
public class FetchStockImpl {
    ...
    public void fetchItlyProductStock() {
        ...
        logger.info("==============拉取bulgaria库存数据开始 " + startDateTime + "=========================");
        ...
        ShangPinPageContent gucciPageContent = getShangPinPageContentByParam(supplierId, "bulgari", 1, Integer.parseInt(pageSize), channel);
        ...
        for (int i = 2; i <= pageNumber; i++) {
            ShangPinPageContent temgucciPageContent = getShangPinPageContentByParam(supplierId, "bulgari", i, Integer.parseInt(pageSize), channel);
        ...
        logger.info("=====需要更新bulgari spProduct Size:" + productDTOAllList.size());
        System.out.println("=====需要更新bulgari spProduct Size:" + productDTOAllList.size());
        //导出尚品库存数据
        ...
        try {
            ...
            if (response.getStatus() == 200) {
                ...
                zhiCai:for (int i = 0; i < zhiCaiSkuResultListSize; i++) {
                    ...
                    if("U".equals(size)){
                        exportSpSkunoAndQty(spSkuNo,IN_STOCK);
                    }

                    String onLinePrice="";
                    Element priceN = doc.select("div.v-align").first();
                    //System.out.println("价格select："+priceN);
                    if(priceN != null && !"".equals(priceN)){
                        String priceArr = priceN.text();
                        onLinePrice = priceArr.substring(0,priceArr.indexOf("€")).trim().replaceAll(",","").trim();
                        byte bytes[] = {(byte) 0xC2,(byte) 0xA0};
                        String UTFSpace = new String(bytes,"utf-8");
                        onLinePrice = onLinePrice.replaceAll(UTFSpace, "&nbsp;").replaceAll("&nbsp;","");
...
//        }
    ...
    public static void main(String[] args) {
        ...
        ProductDTO productDTO = new ProductDTO();
        productDTO.setProductUrl("https://www.bulgari.com/en-fr/products/286878-e.html");
      ...
      // FetchStockImpl fetchStock = new FetchStockImpl();
       // fetchStock.getShangPinPageContentByParam("2018091902059","moncler ",1,20,"www.prada.com");
