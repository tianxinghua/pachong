...
//					orderDTO.setStatus(OrderStatus.NOHANDLE); 
					...
					orderDTO.setExcTime(new Date()); 
					sendMail("订单 "+orderDTO.getSpPurchaseNo()+" spuid等于 "+item_id+" 查询对方库存接口 GetItemStockBySizeMarketPlace 失败,对方返回的信息是："+stockData+",2分钟后会在推送一次。 ");
				}
			}else{
				orderDTO.setStatus(OrderStatus.NOHANDLE); 
				orderDTO.setExcState("0");
	...
	private void sendMail(String messageText){
		try {
			...
			SendMail.sendGroupMail(smtpHost, from, fromUserPassword, to, "!!! Creative99订单推送异常", messageText , messageType);
