...
public class OrderServiceImpl extends AbsOrderService{
	...
	private String getItemStockBySizeMarketPlace(String item_id) throws Exception{
		...
		String returnData = HttpUtil45.postNTAuth(url+getItemStock_interface, param, new OutTimeConfig(1000*60*10,1000*60*10,1000*60*10),user,password);
	...
	private String newOrderMarketPlace(long id_order_mrkp,String barcode,int qty) throws Exception{
		...
		String returnData = HttpUtil45.postNTAuth(url+createOrder_interface, param, new OutTimeConfig(1000*60*10,1000*60*10,1000*60*10),user,password);
	...
	private String setStatusOrderMarketplace(String code,String status) throws Exception{
		...
		String returnData = HttpUtil45.postNTAuth(url+setStatus_interface, param, new OutTimeConfig(1000*60*10,1000*60*10,1000*60*10),user,password);
	...
	private String getStatusOrderMarketplace(String code) throws Exception{
		...
		String returnData = HttpUtil45.postNTAuth(url+getStatus_interface, param, new OutTimeConfig(1000*60*10,1000*60*10,1000*60*10),user,password);
	...
	private void sendMail(String messageText){
		try {
			...
			SendMail.sendGroupMail(smtpHost, from, fromUserPassword, to, "!!! linde订单推送异常", messageText , messageType);
...
//		
		try {
			String result = orderService.getItemStockBySizeMarketPlace("3729136");
