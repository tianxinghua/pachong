...
@Component("creative99ServiceImpl")
public class Creative99ServiceImpl implements IOrderService {
...
//				sendMail("订单 "+orderDTO.getSpPurchaseNo()+" 查询数据库失败,未找到该商品=========== "+skuId);
	...
	private String getItemStockBySizeMarketPlace(String item_id,OrderDTO orderDTO) throws Exception {
		...
		logCommon.loggerOrder(orderDTO, LogTypeStatus.CONFIRM_LOG);
		String returnData = creative99Post(supplierProperties.getCreative99().getUrl()+supplierProperties.getCreative99().getGetItemStockInterface(), param, new OutTimeConfig(1000*60*10,1000*60*10,1000*60*10),supplierProperties.getCreative99().getUser(),supplierProperties.getCreative99().getPassword(),orderDTO);
	...
	private String newOrderMarketPlace(long id_order_mrkp, String barcode, int qty,OrderDTO orderDTO) throws Exception {
		...
		logCommon.loggerOrder(orderDTO, LogTypeStatus.CONFIRM_LOG);
		String returnData = creative99Post(supplierProperties.getCreative99().getUrl()+supplierProperties.getCreative99().getCreateOrderInterface(), param, new OutTimeConfig(1000*60*10,1000*60*10,1000*60*10),supplierProperties.getCreative99().getUser(),supplierProperties.getCreative99().getPassword(),orderDTO);
	...
	private String setStatusOrderMarketplace(String code, String status,OrderDTO orderDTO) throws Exception {
		...
		String returnData = creative99Post(supplierProperties.getCreative99().getUrl()+supplierProperties.getCreative99().getSetStatusInterface(), param, new OutTimeConfig(1000*60*10,1000*60*10,1000*60*10),supplierProperties.getCreative99().getUser(),supplierProperties.getCreative99().getPassword(),orderDTO);
...
//	}
	...
	public static void main(String[] args) {
		Creative99ServiceImpl orderService = new Creative99ServiceImpl();
		OrderDTO orderDTO = new OrderDTO();
//    	orderDTO.setSpPurchaseNo("CGD2016072500097");
		orderDTO.setSpOrderId("201609010000004");
    	orderDTO.setDetail("5464218-2113359693989:1,");
