...
//				sendMail("订单 "+orderDTO.getSpPurchaseNo()+" 查询数据库失败,未找到该商品=========== "+skuId);
	...
	private String getItemStockBySizeMarketPlace(String item_id,OrderDTO orderDTO) throws Exception {
		...
		logCommon.loggerOrder(orderDTO, LogTypeStatus.CONFIRM_LOG);
		String returnData = creative99Post(supplierProperties.getCreative99().getUrl()+supplierProperties.getCreative99().getGetItemStockInterface(), param, new OutTimeConfig(1000*60*2,1000*60*2,1000*60*2),supplierProperties.getCreative99().getUser(),supplierProperties.getCreative99().getPassword(),orderDTO);
	...
	private String newOrderMarketPlace(long id_order_mrkp, String barcode, int qty,OrderDTO orderDTO) throws Exception {
		...
		logCommon.loggerOrder(orderDTO, LogTypeStatus.CONFIRM_LOG);
		String returnData = creative99Post(supplierProperties.getCreative99().getUrl()+supplierProperties.getCreative99().getCreateOrderInterface(), param, new OutTimeConfig(1000*60*2,1000*60*2,1000*60*2),supplierProperties.getCreative99().getUser(),supplierProperties.getCreative99().getPassword(),orderDTO);
