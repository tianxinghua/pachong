...
@Component("carofiglioOrderImpl")
public class CarofiglioOrderImpl implements IOrderService {
     ...
     * @throws Exception
     */
    public String carofiglioPost(String url, Map<String,String> param, OutTimeConfig outTimeConf, String userName, String password,OrderDTO order) throws Exception{
	...
	public void handleConfirmOrder(OrderDTO orderDTO) {
		...
		try {
			String spOrderId = orderDTO.getSpOrderDetailNo();
...
//				sendMail("订单 "+orderDTO.getSpPurchaseNo()+" 查询数据库失败,未找到该商品=========== "+skuId);
	...
	public void handleRefundlOrder(OrderDTO deleteOrder) {
		try {
			String spOrderId = deleteOrder.getSpOrderDetailNo();
			if(spOrderId.contains("-")){
				spOrderId = spOrderId.substring(0, spOrderId.indexOf("-")); 
			}
	...
	private String getItemStockBySizeMarketPlace(String item_id,OrderDTO orderDTO) throws Exception {
		...
		String returnData = carofiglioPost(supplierProperties.getCarofiglio().getUrl()+supplierProperties.getCarofiglio().getGetItemStockInterface(), param, new OutTimeConfig(1000*60*10,1000*60*10,1000*60*10),supplierProperties.getCarofiglio().getUser(),supplierProperties.getCarofiglio().getPassword(),orderDTO);
		return returnData;
	}
	...
	private String newOrderMarketPlace(long id_order_mrkp, String barcode, int qty,OrderDTO orderDTO) throws Exception {
		...
		logCommon.loggerOrder(orderDTO, LogTypeStatus.CONFIRM_LOG);
		String returnData = carofiglioPost(supplierProperties.getCarofiglio().getUrl()+supplierProperties.getCarofiglio().getCreateOrderInterface(), param, new OutTimeConfig(1000*60*10,1000*60*10,1000*60*10),supplierProperties.getCarofiglio().getUser(),supplierProperties.getCarofiglio().getPassword(),orderDTO);
	...
	private String setStatusOrderMarketplace(String code, String status,OrderDTO orderDTO) throws Exception {
		...
		String returnData = carofiglioPost(supplierProperties.getCarofiglio().getUrl()+supplierProperties.getCarofiglio().getSetStatusInterface(), param, new OutTimeConfig(1000*60*10,1000*60*10,1000*60*10),supplierProperties.getCarofiglio().getUser(),supplierProperties.getCarofiglio().getPassword(),orderDTO);
