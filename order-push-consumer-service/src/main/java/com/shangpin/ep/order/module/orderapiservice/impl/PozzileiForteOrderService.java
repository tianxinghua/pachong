...
@Component("pozzileiForteOrderService")
public class PozzileiForteOrderService implements IOrderService {
    ...
    @PostConstruct
    public void init(){
    	cancelUrl = supplierProperties.getPozzileiForte().getCancelUrl();
    	queryOrderUrl = supplierProperties.getPozzileiForte().getQueryOrderUrl();
    	setOrderUrl = supplierProperties.getPozzileiForte().getSetOrderUrl();
    	dBContext = supplierProperties.getPozzileiForte().getDBContext();
    	key = supplierProperties.getPozzileiForte().getKey();
    }
	 ...
	 * @throws Exception
	 */	
	private String pozzileiFortePushOrder(String url, OrderDTO orderDTO,Map<String, String> map)  throws Exception{
	...
	private void createOrder( OrderDTO orderDTO) {
		...
		try {
			 ...
			 map.put("key", key);
			 map.put("sellPrice", "0");
			 rtnData =pozzileiFortePushOrder(setOrderUrl,orderDTO, map);
	...
	private void refundlOrder(OrderDTO deleteOrder) {
		...
		try{
			Map<String, String> map =new HashMap<String, String>();
			 ...
			 map.put("key", key);
			 rtnData2 =pozzileiFortePushOrder(queryOrderUrl,deleteOrder, map);
		...
		if("HO".equals(response.getStatus())){
			try {
				 ...
				 map.put("order_no", deleteOrder.getSpOrderId());
				 map.put("key", key);
				 String rtnData1 =pozzileiFortePushOrder(cancelUrl,deleteOrder, map);
