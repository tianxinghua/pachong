...
@Component("russoCapriServiceImpl")
public class RussoCapriServiceImpl implements IOrderService {
     ...
     * @throws Exception
     */
    public String russoCapriPost(String url, Map<String,String> param, OutTimeConfig outTimeConf, String userName, String password,OrderDTO order) throws Exception{
...
//				sendMail("订单 "+orderDTO.getSpPurchaseNo()+" 查询数据库失败,未找到该商品=========== "+skuId);
	...
	private String getItemStockBySizeMarketPlace(String item_id,OrderDTO orderDTO) throws Exception {
		...
		String returnData = russoCapriPost(supplierProperties.getRussoCapri().getUrl()+supplierProperties.getRussoCapri().getGetItemStockInterface(), param, new OutTimeConfig(1000*60*10,1000*60*10,1000*60*10),supplierProperties.getRussoCapri().getUser(),supplierProperties.getRussoCapri().getPassword(),orderDTO);
		return returnData;
	}
	...
	private String newOrderMarketPlace(long id_order_mrkp, String barcode, int qty,OrderDTO orderDTO) throws Exception {
		...
		logCommon.loggerOrder(orderDTO, LogTypeStatus.CONFIRM_LOG);
		String returnData = russoCapriPost(supplierProperties.getRussoCapri().getUrl()+supplierProperties.getRussoCapri().getCreateOrderInterface(), param, new OutTimeConfig(1000*60*10,1000*60*10,1000*60*10),supplierProperties.getRussoCapri().getUser(),supplierProperties.getRussoCapri().getPassword(),orderDTO);
	...
	private String setStatusOrderMarketplace(String code, String status,OrderDTO orderDTO) throws Exception {
		...
		String returnData = russoCapriPost(supplierProperties.getRussoCapri().getUrl()+supplierProperties.getRussoCapri().getSetStatusInterface(), param, new OutTimeConfig(1000*60*10,1000*60*10,1000*60*10),supplierProperties.getRussoCapri().getUser(),supplierProperties.getRussoCapri().getPassword(),orderDTO);
