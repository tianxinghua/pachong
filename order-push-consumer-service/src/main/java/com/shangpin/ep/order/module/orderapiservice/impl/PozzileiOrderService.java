...
@Component("pozzileiOrderService")
public class PozzileiOrderService implements IOrderService {
    ...
    @PostConstruct
    public void init(){
    	cancelUrl = supplierProperties.getPozzileiParam().getCancelUrl();
    	queryOrderUrl = supplierProperties.getPozzileiParam().getQueryOrderUrl();
    	setOrderUrl = supplierProperties.getPozzileiParam().getSetOrderUrl();
    	dBContext = supplierProperties.getPozzileiParam().getDBContext();
    	key = supplierProperties.getPozzileiParam().getKey();
    }
	/**
	 * 锁库存
	 */
...
//		new PozzileiOrderService().handleConfirmOrder(orderDTO);
   	 ...
   	 * @throws Exception
   	 */	
   	private String pozzileiPushOrder(String url, OrderDTO orderDTO,Map<String, String> map)  throws Exception{
