...
@Component("velaOrderService")
public class VelaOrderService implements IOrderService {
	@Autowired
    LogCommon logCommon;    
    @Autowired
	 ...
	 * @throws Exception
	 */	
	private String velaPushOrder(String url, OrderDTO orderDTO,Map<String, String> map)  throws Exception{
	...
	private void createOrder( OrderDTO orderDTO) {
		...
		try {
			 ...
			 map.put("key", key);
			 map.put("sellPrice", "0");
			 rtnData =velaPushOrder(setOrderUrl,orderDTO, map);
