...
import com.shangpin.ep.order.module.orderapiservice.impl.dto.spinnaker.Parameters;
...
@Component("spinnakerOrderService")
public class SpinnakerOrderService implements IOrderService {
    ...
    @PostConstruct
    public void init(){
    	cancelUrl = supplierProperties.getSpinnakerParam().getCancelUrl();
    	queryOrderUrl = supplierProperties.getSpinnakerParam().getQueryOrderUrl();
    	setOrderUrl = supplierProperties.getSpinnakerParam().getSetOrderUrl();
    	dBContext = supplierProperties.getSpinnakerParam().getDBContext();
    	key = supplierProperties.getSpinnakerParam().getKey();
    }
	 ...
	 * @throws Exception
	 */	
	private String spinnakerPushOrder(String url, OrderDTO orderDTO,Map<String, String> map)  throws Exception{
	...
	private void createOrder( OrderDTO orderDTO) {
		...
		try {
			 ...
			 map.put("key", key);
			 map.put("sellPrice", "0");
			 rtnData =spinnakerPushOrder(setOrderUrl,orderDTO, map);
	...
	private void refundlOrder(OrderDTO deleteOrder) {
		...
		try{
			Map<String, String> map =new HashMap<String, String>();
			 ...
			 map.put("key", key);
			 rtnData2 =spinnakerPushOrder(queryOrderUrl,deleteOrder, map);
		...
		if("HO".equals(response.getStatus())){
			try {
				 ...
				 map.put("order_no", deleteOrder.getSpOrderId());
				 map.put("key", key);
				 String rtnData1 =spinnakerPushOrder(cancelUrl,deleteOrder, map);
