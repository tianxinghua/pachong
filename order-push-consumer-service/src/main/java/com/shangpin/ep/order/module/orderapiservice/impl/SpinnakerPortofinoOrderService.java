...
@Component("spinnakerPortofinoOrderService")
public class SpinnakerPortofinoOrderService implements IOrderService {
    ...
    @PostConstruct
    public void init(){
    	cancelUrl = supplierProperties.getSpinnakerPortofinoParam().getCancelUrl();
    	queryOrderUrl = supplierProperties.getSpinnakerPortofinoParam().getQueryOrderUrl();
    	setOrderUrl = supplierProperties.getSpinnakerPortofinoParam().getSetOrderUrl();
    	dBContext = supplierProperties.getSpinnakerPortofinoParam().getDBContext();
    	key = supplierProperties.getSpinnakerPortofinoParam().getKey();
    }
	 ...
	 * @throws Exception
	 */	
	private String spinnakerPortofinoPushOrder(String url, OrderDTO orderDTO,Map<String, String> map)  throws Exception{
	...
	private void createOrder( OrderDTO orderDTO) {
		...
		try {
			 ...
			 map.put("key", key);
			 map.put("sellPrice", "0");
			 rtnData =spinnakerPortofinoPushOrder(setOrderUrl,orderDTO, map);
	...
	private void refundlOrder(OrderDTO deleteOrder) {
		...
		try{
			...
			Gson gson = new Gson();
			try {
				 String rtnData1 =spinnakerPortofinoPushOrder(cancelUrl,deleteOrder, map);
