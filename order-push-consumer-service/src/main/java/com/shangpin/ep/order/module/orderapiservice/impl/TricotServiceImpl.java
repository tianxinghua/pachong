...
@Component("tricotServiceImpl")
public class TricotServiceImpl implements IOrderService {
     ...
     * @throws Exception
     */
    public String tricotPost(String url, Map<String,String> param, OutTimeConfig outTimeConf, String userName, String password,OrderDTO order) throws Exception{
...
//				sendMail("订单 "+orderDTO.getSpPurchaseNo()+" 查询数据库失败,未找到该商品=========== "+skuId);
	...
	private String getItemStockBySizeMarketPlace(String item_id,OrderDTO orderDTO) throws Exception {
		...
		logCommon.loggerOrder(orderDTO, LogTypeStatus.CONFIRM_LOG);
		String returnData = tricotPost(supplierProperties.getTricot().getUrl()+supplierProperties.getTricot().getGetItemStockInterface(), param, new OutTimeConfig(1000*60*2,1000*60*2,1000*60*2),supplierProperties.getTricot().getUser(),supplierProperties.getTricot().getPassword(),orderDTO);
	...
	private String newOrderMarketPlace(long id_order_mrkp, String barcode, int qty,OrderDTO orderDTO) throws Exception {
		...
		logCommon.loggerOrder(orderDTO, LogTypeStatus.CONFIRM_LOG);
		String returnData = tricotPost(supplierProperties.getTricot().getUrl()+supplierProperties.getTricot().getCreateOrderInterface(), param, new OutTimeConfig(1000*60*2,1000*60*2,1000*60*2),supplierProperties.getTricot().getUser(),supplierProperties.getTricot().getPassword(),orderDTO);
	...
	private String setStatusOrderMarketplace(String code, String status,OrderDTO orderDTO) throws Exception {
		...
		String returnData = tricotPost(supplierProperties.getTricot().getUrl()+supplierProperties.getTricot().getSetStatusInterface(), param, new OutTimeConfig(1000*60*2,1000*60*2,1000*60*2),supplierProperties.getTricot().getUser(),supplierProperties.getTricot().getPassword(),orderDTO);
