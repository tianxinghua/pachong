...
//				sendMail("订单 "+orderDTO.getSpPurchaseNo()+" 查询数据库失败,未找到该商品=========== "+skuId);
	...
	private String getItemStockBySizeMarketPlace(String item_id,OrderDTO orderDTO) throws Exception {
		...
		logCommon.loggerOrder(orderDTO, LogTypeStatus.CONFIRM_LOG);
		String returnData = genteromaPost(supplierProperties.getGenteroma().getUrl()+supplierProperties.getGenteroma().getGetItemStockInterface(), param, new OutTimeConfig(1000*60*2,1000*60*2,1000*60*2),supplierProperties.getGenteroma().getUser(),supplierProperties.getGenteroma().getPassword(),orderDTO);
	...
	private String newOrderMarketPlace(long id_order_mrkp, String barcode, int qty,OrderDTO orderDTO) throws Exception {
		...
		logCommon.loggerOrder(orderDTO, LogTypeStatus.CONFIRM_LOG);
		String returnData = genteromaPost(supplierProperties.getGenteroma().getUrl()+supplierProperties.getGenteroma().getCreateOrderInterface(), param, new OutTimeConfig(1000*60*2,1000*60*2,1000*60*2),supplierProperties.getGenteroma().getUser(),supplierProperties.getGenteroma().getPassword(),orderDTO);
	...
	private String setStatusOrderMarketplace(String code, String status,OrderDTO orderDTO) throws Exception {
		...
		String returnData = genteromaPost(supplierProperties.getGenteroma().getUrl()+supplierProperties.getGenteroma().getSetStatusInterface(), param, new OutTimeConfig(1000*60*2,1000*60*2,1000*60*2),supplierProperties.getGenteroma().getUser(),supplierProperties.getGenteroma().getPassword(),orderDTO);
