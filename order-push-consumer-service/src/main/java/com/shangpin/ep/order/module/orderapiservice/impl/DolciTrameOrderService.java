...
@Component("dolciTrameOrderService")
public class DolciTrameOrderService implements IOrderService {
    ...
    @PostConstruct
    public void init(){
    	cancelUrl = supplierProperties.getDolciTrameParam().getCancelUrl();
    	queryOrderUrl = supplierProperties.getDolciTrameParam().getQueryOrderUrl();
    	setOrderUrl = supplierProperties.getDolciTrameParam().getSetOrderUrl();
    	dBContext = supplierProperties.getDolciTrameParam().getDBContext();
    	key = supplierProperties.getDolciTrameParam().getKey();
    }
	 ...
	 * @throws Exception
	 */	
	private String dolciTramePushOrder(String url, OrderDTO orderDTO,Map<String, String> map)  throws Exception{
	...
	private void createOrder( OrderDTO orderDTO) {
		...
		try {
			 ...
			 map.put("key", key);
			 map.put("sellPrice", "0");
			 rtnData =dolciTramePushOrder(setOrderUrl,orderDTO, map);
	...
	private void refundlOrder(OrderDTO deleteOrder) {
		...
		try{
			...
			Gson gson = new Gson();
			try {
				 String rtnData1 =dolciTramePushOrder(cancelUrl,deleteOrder, map);
