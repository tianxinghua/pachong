...
public class MontiOrderService implements IOrderService {
    ...
    @PostConstruct
    public void init(){
    	cancelUrl = supplierProperties.getMontiParam().getCancelUrl();
    	queryOrderUrl = supplierProperties.getMontiParam().getQueryOrderUrl();
    	setOrderUrl = supplierProperties.getMontiParam().getSetOrderUrl();
    	dBContext = supplierProperties.getMontiParam().getDBContext();
    	key = supplierProperties.getMontiParam().getKey();
    }
    /**
	 * 锁库存
	 */
	...
	private void refundlOrder(OrderDTO deleteOrder) {
		...
		try{
			Map<String, String> map =new HashMap<String, String>();
			 ...
			 map.put("key", key);
			 rtnData2 =montiPushOrder(queryOrderUrl,deleteOrder, map);
