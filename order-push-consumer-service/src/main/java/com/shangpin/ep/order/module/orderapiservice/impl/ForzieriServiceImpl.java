...
//    	String client = "NTkyZThjNGI2NGY2NTY1";
//    	String secret = "3db7c25746f4a11bd8f877b1132dc4a5e739535e";
//	}
...
//									1000 * 60));
     ...
     * @throws Exception
     */
    public String paoloPost(String url, Map<String,String> param, OutTimeConfig outTimeConf, String userName, String password,OrderDTO order) throws Exception{
...
//				sendMail("订单 "+orderDTO.getSpPurchaseNo()+" 查询数据库失败,未找到该商品=========== "+skuId);
	...
	private String getItemStockBySizeMarketPlace(String item_id,OrderDTO orderDTO) throws Exception {
		...
		logCommon.loggerOrder(orderDTO, LogTypeStatus.CONFIRM_LOG);
		String returnData = paoloPost(supplierProperties.getPaolo().getUrl()+supplierProperties.getPaolo().getGetItemStockInterface(), param, new OutTimeConfig(1000*60*2,1000*60*2,1000*60*2),supplierProperties.getPaolo().getUser(),supplierProperties.getPaolo().getPassword(),orderDTO);
	...
	private String newOrderMarketPlace(long id_order_mrkp, String barcode, int qty,OrderDTO orderDTO) throws Exception {
		...
		logCommon.loggerOrder(orderDTO, LogTypeStatus.CONFIRM_LOG);
		String returnData = paoloPost(supplierProperties.getPaolo().getUrl()+supplierProperties.getPaolo().getCreateOrderInterface(), param, new OutTimeConfig(1000*60*2,1000*60*2,1000*60*2),supplierProperties.getPaolo().getUser(),supplierProperties.getPaolo().getPassword(),orderDTO);
	...
	private String setStatusOrderMarketplace(String code, String status,OrderDTO orderDTO) throws Exception {
		...
		String returnData = paoloPost(supplierProperties.getPaolo().getUrl()+supplierProperties.getPaolo().getSetStatusInterface(), param, new OutTimeConfig(1000*60*2,1000*60*2,1000*60*2),supplierProperties.getPaolo().getUser(),supplierProperties.getPaolo().getPassword(),orderDTO);
