...
public class ZitafabianiServiceImpl implements IOrderService {
     ...
     * @throws Exception
     */
    public String zitafabianiPost(String url, Map<String,String> param, OutTimeConfig outTimeConf, String userName, String password,OrderDTO order) throws Exception{
...
//				sendMail("订单 "+orderDTO.getSpPurchaseNo()+" 查询数据库失败,未找到该商品=========== "+skuId);
	...
	private String getItemStockBySizeMarketPlace(String item_id,OrderDTO orderDTO) throws Exception {
		...
		String returnData = zitafabianiPost(supplierProperties.getZitafabiani().getUrl()+supplierProperties.getZitafabiani().getGetItemStockInterface(), param, new OutTimeConfig(1000*60*10,1000*60*10,1000*60*10),supplierProperties.getZitafabiani().getUser(),supplierProperties.getZitafabiani().getPassword(),orderDTO);
		return returnData;
	}
	...
	private String newOrderMarketPlace(long id_order_mrkp, String barcode, int qty,OrderDTO orderDTO) throws Exception {
		...
		logCommon.loggerOrder(orderDTO, LogTypeStatus.CONFIRM_LOG);
		String returnData = zitafabianiPost(supplierProperties.getZitafabiani().getUrl()+supplierProperties.getZitafabiani().getCreateOrderInterface(), param, new OutTimeConfig(1000*60*1,1000*60*1,1000*60*1),supplierProperties.getZitafabiani().getUser(),supplierProperties.getZitafabiani().getPassword(),orderDTO);
	...
	private String setStatusOrderMarketplace(String code, String status,OrderDTO orderDTO) throws Exception {
		...
		String returnData = zitafabianiPost(supplierProperties.getZitafabiani().getUrl()+supplierProperties.getZitafabiani().getSetStatusInterface(), param, new OutTimeConfig(1000*60*10,1000*60*10,1000*60*10),supplierProperties.getZitafabiani().getUser(),supplierProperties.getZitafabiani().getPassword(),orderDTO);
