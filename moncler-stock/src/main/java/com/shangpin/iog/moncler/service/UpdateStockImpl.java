...
public class UpdateStockImpl extends FetchStockImpl {
    ...
    FetchStockImpl fobject=new  FetchStockImpl();
    ...
    public void fetchItlyProductStock(){
        ...
        String csvFilePath = filePath +"moncler-qty-"+ todayDateStr+"-2.csv";
        if(!new File(csvFilePath).exists()){
            csvFilePath = filePath +"moncler-qty-"+ todayDateStr+"-1.csv";
            if(!new File(csvFilePath).exists()){
                csvFilePath = filePath +"moncler-qty-"+ yesterdayDateStr+"-2.csv";
                if(!new File(csvFilePath).exists()){
                    csvFilePath = filePath +"moncler-qty-"+ yesterdayDateStr+"-1.csv";
                }
            }
        }
        try {
