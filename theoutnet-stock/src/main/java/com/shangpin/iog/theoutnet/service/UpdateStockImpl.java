...
import com.shangpin.iog.theoutnet.dto.ApiResponseBody;
import com.shangpin.iog.theoutnet.dto.SpSkuNoDTO;
import com.shangpin.iog.theoutnet.dto.ZhiCaiSkuHttpDTO;
import com.shangpin.iog.theoutnet.dto.ZhiCaiSkuStock;
...
public class UpdateStockImpl extends FetchStockImpl {
    ...
    public void fetchItlyProductStock(){
        ...
        System.out.println("============更新theoutnet库存数据库开始 "+startDateTime+"=========================");
        logger.info("==============更新theoutnet库存数据库开始 "+startDateTime+"=========================");
        ...
        String csvFilePath = filePath +"theoutnet-qty-"+ todayDateStr+"-2.csv";
        if(!new File(csvFilePath).exists()){
            csvFilePath = filePath +"theoutnet-qty-"+ todayDateStr+"-1.csv";
            if(!new File(csvFilePath).exists()){
                csvFilePath = filePath +"theoutnet-qty-"+ yesterdayDateStr+"-2.csv";
                if(!new File(csvFilePath).exists()){
                    csvFilePath = filePath +"theoutnet-qty-"+ yesterdayDateStr+"-1.csv";
                }
            }
        }
        ...
        logger.info("===================更新theoutnet库存数据库结束 "+endtDateTime+"=========================");
        System.out.println("=================更新theoutnet库存数据库结束 "+endtDateTime+"=========================");
        /*logger.info("===================准备发送邮件 =========================");
    ...
    public static void main(String[] args) {
        ...
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd");
        String yesterdayDateStr = simpleDateFormat.format(yesterDate);
        String csvFilePath = filePath +"theoutnet-qty-"+ yesterdayDateStr+".csv";
        try {
