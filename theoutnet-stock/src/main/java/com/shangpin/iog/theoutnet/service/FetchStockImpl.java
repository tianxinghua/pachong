package com.shangpin.iog.theoutnet.service;

import com.google.gson.JsonArray;
import com.google.gson.JsonObject;
import com.google.gson.JsonParser;
import com.shangpin.iog.theoutnet.dto.*;
...
public class FetchStockImpl {
    ...
    public void fetchItlyProductStock(){
        ...
        System.out.println("============拉取theoutnet库存数据开始 "+startDateTime+"=========================");
        logger.info("==============拉取theoutnet库存数据开始 "+startDateTime+"=========================");
        ...
        String temFilePath = filePath + "theoutnet-qty-"+todayStr+"-1.csv";
        File fristFile = new File(temFilePath);
        if(fristFile.exists()){
            temFilePath = filePath + "theoutnet-qty-"+todayStr+"-2.csv";
        }
        String priceFilePath = filePath + "theoutnet-price-"+todayStr+"-1.csv";
        File secondFile = new File(priceFilePath);
        if(secondFile.exists()){
            priceFilePath = filePath + "theoutnet-price-"+todayStr+"-2.csv";
        }
        System.out.println("文件保存目录："+temFilePath);
        logger.info("文件保存目录："+temFilePath);
        System.out.println("价格文件保存目录："+temFilePath);
        logger.info("价格文件保存目录："+temFilePath);
        try {
        ...
        //获取第一页商品数据
        ShangPinPageContent selfridgesPageContent = getShangPinPageContentByParam(supplierId,"","www.theoutnet.com",1, Integer.parseInt(pageSize));
        ...
        for (int i = 2; i <= pageNumber; i++) {
            ShangPinPageContent temselfridgesPageContent = getShangPinPageContentByParam(supplierId,"","www.theoutnet.com", i, Integer.parseInt(pageSize));
            if(temselfridgesPageContent!=null){
                productDTOAllList.addAll(temselfridgesPageContent.getZhiCaiResultList());
            }else{ //请求失败重新 再次请求
                temselfridgesPageContent = getShangPinPageContentByParam(supplierId,"","www.theoutnet.com", i, Integer.parseInt(pageSize));
                if(temselfridgesPageContent!=null){
                    productDTOAllList.addAll(temselfridgesPageContent.getZhiCaiResultList());
                }
            }
        ...
        logger.info("=====需要更新theoutnet spProduct Size:"+productDTOAllList.size());
        System.out.println("=====需要更新theoutnet spProduct Size:"+productDTOAllList.size());
        //导出尚品库存数据
        ...
        logger.info("===================拉取theoutnet库存数据结束 "+endtDateTime+"=========================");
        System.out.println("=================拉取theoutnet库存数据结束 "+endtDateTime+"=========================");
            ...
            if (response.getStatus()==200) {
                ...
                for (int j = 0; j < spSkuSize; j++) {
                    ...
                    for (int i = 0; i < pageSize; i++) {
                        String sizeText = sizeElements.get(i).text();
                        String qtyDesc=sizeElements.get(i).attr("class");
                        ...
                        try {
                            String sizeValue = "";
                            if (sizeElements.get(i).select(".sizeLabel").text().equals("--")){
                                sizeValue="U";
                        ...
                        if (StringUtils.isEmpty(salePrice)){
                            salePrice=doc.select(".item-content__price").select(".price").select("span.value").first().text().trim().replace(",","");
                            price=salePrice;
                        }
                        ...
                        if (marketPrice != null) {
                            ...
                            if (temElementPrice != spMarketPrice) { //价格发生改变
                                updateSpSkuMarketPrice(skuDTO.getSupplierSkuNo(),price,salePrice);
...
//        }
    ...
    protected static void sendMail(String filePath){
        System.out.println("准备发送的文件："+filePath);
        String time=filePath.split("price-")[1];
        time=time.split(".csv")[0];
        ...
        try
        {
            ...
            message.setSubject(time+"-"+bdl.getString("uri")+"修改了价格的商品信息");
            ...
            //DataSource dataSource = new FileDataSource(bdl.getString("csvFilePath")+"lkbennett-price-"+yesterdayDateStr+".csv");
            DataSource dataSource = new FileDataSource(filePath);
            //使用datasource实例化datahandler
            DataHandler dataHandler = new DataHandler(dataSource);
            bodyPart.setDataHandler(dataHandler);
            //设置附件标题，使用MimeUtility进行名字转码，否则接收到的是乱码
            bodyPart.setFileName(javax.mail.internet.MimeUtility.encodeText(time+"修改了价格的商品信息"+".csv"));
            multipart.addBodyPart(bodyPart);
    ...
    protected boolean getFileToEmail(){
        ...
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd");
        String yesterdayDateStr = simpleDateFormat.format(yesterDate);
        String todayStr = simpleDateFormat.format( new Date(new Date().getTime()));
        String fileName=bdl.getString("csvFilePath")+"theoutnet-price-"+todayStr+"-1.csv";
        File file=new File(bdl.getString("csvFilePath")+"theoutnet-price-"+todayStr+"-1.csv");
        if (!(file.exists())){
            file=new File(bdl.getString("csvFilePath")+"theoutnet-price-"+yesterdayDateStr+"-2.csv");
            fileName=bdl.getString("csvFilePath")+"theoutnet-price-"+yesterdayDateStr+"-2.csv";

            if (!file.exists()){
                file=new File(bdl.getString("csvFilePath")+"theoutnet-price-"+yesterdayDateStr+"-1.csv");
                fileName=bdl.getString("csvFilePath")+"theoutnet-price-"+yesterdayDateStr+"-1.csv";
            }
        }
        try {
    ...
    public static void main(String[] args) {
        ...
        productDTO.setProductUrl("https://www.theoutnet.com/en-gb/shop/product/medium-knit_cod1016843419909470.html#dept=INTL_Brunello_Cucinelli_DESIGNERS");
