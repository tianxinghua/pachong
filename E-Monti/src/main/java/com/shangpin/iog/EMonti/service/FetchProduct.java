...
import com.shangpin.iog.EMonti.dto.ProductDTO;
import com.shangpin.iog.EMonti.dto.ProductSkuDTO;
		...
		try {
			...
			for(Map map : objects){	
				try {
					...
					System.out.println("product message==="+ product.toString());
					//获取价格和库存
					String skuStock = "";
					...
					if(null != category_ids && category_ids.length>0){
						for(String catid :category_ids){
							...
							if(!category.equals(categoryObj.get("name").toString())){//只取一个品类
								category = categoryObj.get("name").toString();
							}
					...
					sku.setSkuId(product.get("product_id").toString());//spu 当SKU 为了能在airshop中展示 以便供货商可以去找货
					sku.setSpuId(product.get("product_id").toString());
					sku.setProductName(product.get("name").toString());
					sku.setMarketPrice(product.get("price").toString());
					sku.setSalePrice(product.get("price").toString());
					sku.setSupplierPrice(product.get("cost").toString());
					sku.setProductCode(product.get("sku").toString());//sku 当货号
...
//					}
					...
					try {
						...
						picMap.put("product",product.get("sku").toString());
...
//						productFetchService.savePicture(supplierId, null, sku.getSkuId(), picList);
					...
					//产地
					spu.setProductOrigin(product.get("country_of_manufacture").toString());
...
//					}
					...
					productDTO.setProductCode(sku.getProductCode());
					productDTO.setColor(sku.getColor());
					productDTO.setProductDescription(sku.getProductDescription());
					productDTO.setProductSkuDTO(skuDTO);
					productDTO.setImageUrls(picList);

					supp.setData(objectMapper.writeValueAsString(productDTO));
					pushMessage(null);
