...
public class FetchStockImpl {
    ...
    public void fetchItlyProductStock() {
        ...
        System.out.println("============拉取theKooples库存数据开始 " + startDateTime + "=========================");
        logger.info("==============拉取theKooples库存数据开始 " + startDateTime + "=========================");
        ...
        String temFilePath = filePath + "theKooples-qty-" + todayStr + ".csv";
        String priceFilePath = filePath + "theKooples-price-"+todayStr+".csv";
            ...
            if (response.getStatus() == 200) {
                ...
                zhiCai:for (int i = 0; i < zhiCaiSkuResultListSize; i++) {
                    ...
                    for(Element script : scripts) {
                        ...
                        Element element = doc.select("#product_addtocart_form > div.price-info > div ").first();
                        String onLinePrice = element.text();
                        onLinePrice = onLinePrice.trim().substring(0,onLinePrice.length()-1).replaceAll(",",".").trim();
                        byte bytes[] = {(byte) 0xC2,(byte) 0xA0};
                        String UTFSpace = new String(bytes,"utf-8");
                        onLinePrice = onLinePrice.replaceAll(UTFSpace, "&nbsp;").replaceAll("&nbsp;","");
                        System.out.println("线上价格："+onLinePrice);
                    ...
                    if(i==zhiCaiSkuResultListSize-1){
                        copyFile();
                    }
                }
        }else{
...
//        }
    ...
    public static void sendMail(){
        ...
        try
        {
           ...
           /* long dayTime = 1000*3600*24l;
            ...
            DataSource dataSource = new FileDataSource(bdl.getString("csvFilePath")+"theKooples-price-"+todayStr+".csv");
    ...
    protected void getFileToEmail(){
       /* long dayTime = 1000*3600*24l;
        ...
        String fileName=bdl.getString("csvFilePath")+"theKooples-price-"+todayDateStr+".csv";
        File file=new File(bdl.getString("csvFilePath")+"theKooples-price-"+todayDateStr+".csv");
        try {
