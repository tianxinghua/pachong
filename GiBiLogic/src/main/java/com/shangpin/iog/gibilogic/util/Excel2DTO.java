package com.shangpin.iog.gibilogic.util;
...
public class Excel2DTO {
	...
	public static <T> List<T> toDTO(String filePath, Integer sheetNum,
	...
	private static <T> T fillDTO(T t, XSSFRow row, Short[] needColsNo) {
		try {
			Field[] fields = t.getClass().getDeclaredFields();
			for (int i = 0; i < needColsNo.length; i++) {
				fields[i].setAccessible(true);
				fields[i].set(t, StringUtils.isBlank(getStrinCellValue(row.getCell(needColsNo[i])))?"":getStrinCellValue(row.getCell(needColsNo[i])));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private static String getStrinCellValue(XSSFCell xssfCell) {
